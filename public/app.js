...
// 4. Automatically select all categories by default
...
let currentSuspects = ['Player 1', 'Player 2', 'Player 3']; // Minimum 3 players
...
// 1. Renders all saved groups on the first page
...
function addSuspectField() {
  // 3. Maximum 20 players constraint
...
function removeSuspect(idx) {
  // 3. Minimum 3 players constraint
...
// 4. Toggle all categories (Unselect all / Select all)
function toggleAllCategories() {
  if (selectedCategories.length === ALL_CATEGORIES_LIST.length) {
    selectedCategories = []; // Unselect option
  } else {
    selectedCategories = [...ALL_CATEGORIES_LIST]; // Select all
  }
  renderCategoriesUI();
}

// 4. Toggle individual category
...
async function startGame() {
  // 3. Minimum 3 players required check
  if (currentSuspects.length < 3) return alert("Minimum 3 players required.");
  ...
  // 2. Automatically save created group for next and future games
  ...
  // Non-repeating Mr White selection logic
